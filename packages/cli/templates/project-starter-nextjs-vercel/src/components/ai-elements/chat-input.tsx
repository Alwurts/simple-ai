...
export function ChatInput({
	...
	return (
		...
		>
			<InputGroup
				className={cn(
					"focus-within:ring-1 focus-within:ring-ring rounded-2xl bg-background dark:bg-background",
					className,
				)}
				{...props}
			>
	...
	return (
		<>
			...
			{!editor ? (
				<div
					className={cn(
						"w-full h-full max-h-48 px-4 pt-4 pb-2",
						"min-h-[48px] flex items-center",
						className,
					)}
				>
					<Skeleton className="w-full h-5 rounded bg-muted/20" />
				</div>
			) : (
				<EditorContent
					editor={editor}
...
// biome-ignore lint/suspicious/noExplicitAny: Required for type inference
type ConfigToField<Config extends MentionConfig<any>> = Config extends MentionConfig<infer T>
	? { [K in Config["type"]]: T[] }
	: never;
