	...
	return (
		<div className="hidden group-data-[view=code]/block-view-wrapper:hidden md:h-[--height] lg:flex">
			<div className="relative grid w-full gap-4">
				<div className="absolute inset-0 right-4 bg-muted rounded-xl" />
				<ResizablePanelGroup
				...
				>
					...
					<ResizableHandle className="after:bg-border relative w-3 bg-transparent p-0 after:absolute after:top-1/2 after:right-0 after:h-8 after:w-[6px] after:-translate-x-px after:-translate-y-1/2 after:rounded-full after:transition-all after:hover:h-10 pointer-events-none opacity-0 md:pointer-events-auto md:opacity-100" />
				...
				<CollapsibleContent>
					<SidebarMenuSub className="m-0 w-full translate-x-0 border-none p-0">
						{item.children.map((subItem, key) => (
							<Tree key={`${item.name}-${key}`} item={subItem} index={index + 1} />
						))}
					</SidebarMenuSub>
				</CollapsibleContent>
			</Collapsible>
